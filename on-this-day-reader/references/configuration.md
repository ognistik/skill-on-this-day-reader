...
```bash
python3 scripts/configure.py note-url "bear://x-callback-url/create?title={title}&text={analysis}&open_note=no&tags=0-inbox"
```
...
```json
{
  ...
  "note": {
    "enabled": false,
    "url_template": "bear://x-callback-url/create?title={title}&text={analysis}&open_note=no&tags=0-inbox",
